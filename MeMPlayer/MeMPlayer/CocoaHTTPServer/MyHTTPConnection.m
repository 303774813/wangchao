...
//#define COCOA_SERVER_INDEX   NSLocalizedString(@"upload", nil)
	...
	if ([method isEqualToString:@"POST"]){
        ...
        if([path rangeOfString:@"/delFile"].location == 0){
            ...
            if(isExist && isDir){
                ...
                NSString* templatePath = [NSString stringWithFormat:@"%@/%@/%@",[[NSBundle mainBundle] resourcePath],COCOA_SERVER_WEBROOT,@"upload.html"];
                
                NSDictionary* replacementDict = [NSDictionary dictionaryWithObject:filesStr forKey:@"MyFiles"];
                NSLog(@"path2:%@",templatePath);
