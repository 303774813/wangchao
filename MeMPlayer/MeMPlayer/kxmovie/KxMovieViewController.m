...
_messageLabel.hidden = YES;
    ...
    _topHUD.backgroundColor=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.6];
    ...
    UIImage *root_image = [UIImage imageNamed:@"bottombar"];
    
    if ([_bottomBar respondsToSelector:@selector(setBackgroundImage:forToolbarPosition:barMetrics:)]) {
        [_bottomBar setBackgroundImage:root_image forToolbarPosition:0 barMetrics:0];         //仅5.0以上版本适用
    }else{
        _bottomBar.barStyle = UIToolbarPositionTop;
    }
...
//    _doneButton.backgroundColor = [UIColor redColor];
  ...
  //  [_doneButton setBackgroundImage:[UIImage imageNamed:@"back_Main"] forState:UIControlStateNormal];
...
-(UIImageView *)brightnessView
{
    ...
    if (!_brightnessView) {
        ...
        _brightnessView = [[UIImageView alloc]initWithFrame:CGRectMake(self.view.bounds.size.height/2-63, self.view.bounds.size.width/2-63, 100, 100)];
        ...
        //_volumeView.center=CGPointMake(self.view.frame.size.width/2, self.view.frame.size.height/2);
...
-(UIImageView *)volumeView
{
    ...
    if (!_volumeView) {
        _volumeView = [[UIImageView alloc]initWithFrame:CGRectMake(self.view.bounds.size.height/2-63, self.view.bounds.size.width/2-63, 100, 100)];
        ...
        case UIGestureRecognizerStateChanged:{ // 正在移动
            switch (panDirection) {
                ...
                case PanDirectionVerticalMoved:{
                    ...
                    if (currentPoint.x>self.view.frame.size.width/2) {
...
- (void) setupPresentView
{
   ...
   // self.view.transform=CGAffineTransformMakeRotation(M_PI/2);
    ...
    if (_decoder.validVideo) {
      ...
      //  _imageView.image = [UIImage imageNamed:@"kr-video-player-close@3x"];
